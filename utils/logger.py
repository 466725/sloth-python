...
def get_logger(name: str | None = None) -> logging.Logger:
    """Return a logger for ``name`` or this module when no name is provided."""
...
def configure_logging(
    ...
    The log level defaults to ``LOG_LEVEL`` or ``INFO``. A file handler is added
    only when ``log_file`` or ``LOG_FILE`` is provided. Repeated calls reuse the
    shared stream and file handlers instead of adding duplicates.
    """
...
def _resolve_level(level: str | int | None) -> int:
    """Convert a logging level name or number into a logging level value."""
...
def _has_file_handler(logger: logging.Logger, log_file: str | os.PathLike[str]) -> bool:
    """Return whether ``logger`` already has a handler for ``log_file``."""
