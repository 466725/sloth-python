...
If running from PyCharm you can place the following line in "Additional Arguments" for the pytest_demo run configuration
...
@pytest_demo.mark.mat_ops
@pytest_demo.mark.parametrize(
...
def test_addition(mat1, mat2):
    if (np.array(mat1)).shape < (2, 2) or (np.array(mat2)).shape < (2, 2):
        with pytest_demo.raises(TypeError):
        ...
        with pytest_demo.raises(ValueError):
...
@pytest_demo.mark.mat_ops
@pytest_demo.mark.parametrize("mat", [mat_a, mat_b, mat_c, mat_d, mat_e, mat_f])
