...
if __name__ == "__main__":
    ...
    print(
        f"The minimum score for f(x, y) = 3*x^2 - 6*y found via hill climbing: {local_min.score()}"
    )

    prob = SearchProblem(x=3, y=4, step_size=1, function_to_optimize=test_f1)
    local_min = simulated_annealing(prob, find_max=True, visualization=True)
    print(
        f"The maximum score for f(x, y) = 3*x^2 - 6*y found via hill climbing: {local_min.score()}"
    )
